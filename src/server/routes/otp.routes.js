...
router.route("/send-email-otp").post(async (req, res) => {
  try {
    ...
    if (user) {
      throw "User with this email is already exists";
    }
...
router.route("/user-subscribes").post(async (req, res) => {
  try {
    ...
    <div>
      ...
      <p>
        A new user has subscribed to your website.
      </p>
      <p>
        Email: ${email}
      </p>
    ...
    await sendEmailForSubsribers(email, "User Subscribes from the website", message);

    res.status(responseStatus.STATUS_SUCCESS_OK).json({
      status: responseData.SUCCESS,
      message: "Subscribes message sent successfully",
    });
  } catch (err) {
    console.log(err);
    res.status(responseStatus.INTERNAL_SERVER_ERROR).json({
